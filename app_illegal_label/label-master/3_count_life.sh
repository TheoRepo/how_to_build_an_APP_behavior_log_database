...
mobile_id_end_no
from 
profile.dws_app_all_20210530 a 
left join 
(select app_name, app_type as app_strategy from profile.dws_app_regexp_20211030 where app_type = '生活类' group by app_name, app_type) b
...
if [[ $? != 0 ]];then
    msg="定时调度【手机号生活类app使用总量表(ga.interface_app_cnt_by_strategy_20210930)】脚本3_count_life.sh跑数失败"
