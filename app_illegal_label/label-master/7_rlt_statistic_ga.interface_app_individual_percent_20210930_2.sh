...
select CONCAT(app_strategy, '类app高比例使用个数统计表_单人数据量最大值'),max(count) from
...
if [[ $? != 0 ]];then
    msg="定时调度【app使用情况数据统计】脚本7_rlt_statistic_ga.interface_app_individual_percent_20210930_2.sh跑数失败"
...
msg2="定时调度【app使用情况数据统计】7_rlt_statistic_ga.interface_app_individual_percent_20210930_2.sh脚本跑数成功！！"
