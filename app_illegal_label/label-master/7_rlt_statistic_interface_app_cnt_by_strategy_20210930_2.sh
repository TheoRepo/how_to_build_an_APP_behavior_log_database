...
select CONCAT(app_strategy, '类app使用个数统计表_单人数据量最小值') as label, min(cast(count as int)) cnt from ga.interface_app_cnt_by_strategy_20210930 group by app_strategy;
"
cd /home/qianyu/ && bash spark_sql.sh "app" "$sql_part"

if [[ $? != 0 ]];then
    msg="定时调度【app使用情况数据统计】脚本7_rlt_statistic_interface_app_cnt_by_strategy_20210930_2.sh跑数失败"
...
msg2="定时调度【app使用情况数据统计】7_rlt_statistic_interface_app_cnt_by_strategy_20210930_2.sh脚本跑数成功！！"
