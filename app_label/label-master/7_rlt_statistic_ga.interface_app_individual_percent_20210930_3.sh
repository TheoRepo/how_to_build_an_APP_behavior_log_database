...
select CONCAT(app_strategy, '类app高比例使用个数统计表_单人数据量最小值'),min(count) from
(select a.mobile_id, a.count, a.app_strategy 
from ga.interface_app_cnt_by_strategy_20210930 a
left join 
(select mobile_id from ga.interface_app_individual_percent_20210930 where percent = '100%') b
on 
a.mobile_id = b.mobile_id
where b.mobile_id is not null) c
group by app_strategy;
"

cd /home/qianyu/ && bash spark_sql.sh "app" "$sql_part"

if [[ $? != 0 ]];then
    msg="定时调度【app使用情况数据统计】脚本7_rlt_statistic_ga.interface_app_individual_percent_20210930_3.sh跑数失败"
...
msg2="定时调度【app使用情况数据统计】7_rlt_statistic_ga.interface_app_individual_percent_20210930_3.sh脚本跑数成功！！"
