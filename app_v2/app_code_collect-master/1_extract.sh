...
(
    ...
    case when b.virtual_id is not null then regexp_extract(a.msg,b.virtual_id,1)
    else null
    end as virtual_id,
    the_date,
    version_no
    from
        (
            ...
            msg,
            the_date
            from ${source_table} where the_date='${the_date}' 
        )a
        inner join
        (
            select
            ...
            behavior_index
            from ${regexp_table} 
            where version_no='${version_no}' and (the_date = '2021-05-25' or the_date = '2021-05-20' or the_date = '2021-07-09')
        )b 
        on a.suspected_app_name = b.suspected_app_name 
        where if(b.app_name is not null,a.app_name = b.app_name,1=1) 
        and if(b.posi_regexp is not null,a.msg regexp b.posi_regexp,1=1) 
        ...
        from
            (
             select
                ...
                the_date
                from ${source_table} 
                where the_date = '${the_date}' and app_name<>'未识别' 
                )a
            inner join
                (
                select
                ...
                behavior_index
                from ${regexp_table}
                where version_no='${version_no}' and (the_date = '2021-06-08' or the_date = '2021-05-12' or the_date = '2021-07-19' or the_date ='2021-06-16')
                )b 
            on a.app_name = b.app_name
            where if(b.suspected_app_name is not null,a.suspected_app_name = b.suspected_app_name,1=1) 
            and if(b.posi_regexp is not null,a.msg regexp b.posi_regexp,1=1) 
            and if(b.nege_regexp is not null,a.msg not regexp b.nege_regexp,1=1)
) c
...
if [[ $? != 0 ]];then
    msg="定时调度【${version_no}版本规则对${the_date}短文本的APP行为信息抽取】1_extract.sh脚本跑数失败了55555"
