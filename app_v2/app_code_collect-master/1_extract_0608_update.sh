source /etc/profile
source  ~/.bash_profile
version_no=$1
spark_sql_path=$2
label=$3
source_table=$4
regexp_table=$5
notice_ids="18801036793"




sql_part="

Insert overwrite TABLE profile${label}.dwd_app_20211030 partition(the_date,version_no)
select row_key,mobile_id,app_name,app_type,
case when behavior='' then null else behavior end as behavior,
event_time,
case when virtual_id='' then null else virtual_id end as virtual_id,
the_date,version_no
from 
(select row_key,mobile_id,app_name,app_type,max(behavior) as behavior,event_time,max(virtual_id) as virtual_id,the_date,version_no
from 
(select
/*+mapjoin(b)*/
a.row_key,
a.mobile_id,
b.app_index as app_name,
b.type_index as app_type,
b.behavior_index as behavior,
a.event_time,
case
when b.virtual_id is not null then regexp_extract(a.msg,b.virtual_id,1)
else null
end as virtual_id,
a.the_date,
version_no
from
(select
row_key,
mobile_id,
event_time,
app_name,
suspected_app_name,
msg,
the_date
from ${source_table} where the_date >= date_sub(current_date(),15) and app_name<>'未识别' )a
inner join
(
select
app_name,
app_type,
suspected_app_name,
posi_regexp,
nege_regexp,
mapping_app_name,
behavior,
virtual_id,
version_no,
app_index,
type_index,
behavior_index
from ${regexp_table} where the_date='2021-06-08' and version_no='${version_no}'
)b on a.app_name = b.app_name
where if(b.suspected_app_name is not null,a.suspected_app_name = b.suspected_app_name,1=1) and if(b.posi_regexp is not null,a.msg regexp b.posi_regexp,1=1) and if(b.nege_regexp is not null,a.msg not regexp b.nege_regexp,1=1)
)t
group by row_key,mobile_id,app_name,app_type,event_time,the_date,version_no
)t
distribute by rand();
"

bash /home/${spark_sql_path}/spark_sql.sh "appv2" "$sql_part"

if [[ $? != 0 ]];then
    msg="定时调度【${version_no}版本规则对${the_date}短文本的APP行为信息抽取】1_extract_0608_update.sh脚本跑数失败了55555"
    curl -X POST -H 'Content-Type: application/json' -d '{"accessToken": "faed9fc12a3a47bae96b94a068e2066642d0a366b752565d80465fe2a7203b9a","content": "'${msg}'", "mobiles":"'${notice_ids}'"}' "http://10.10.15.52:8888/conch/notice/ding"
    exit 1
fi

msg2="定时调度【${version_no}版本规则对${the_date}短文本的APP行为信息抽取】跑数成功~！！！"
curl -X POST -H 'Content-Type: application/json' -d '{"accessToken": "faed9fc12a3a47bae96b94a068e2066642d0a366b752565d80465fe2a7203b9a","content": "'${msg2}'", "mobiles":"'${notice_ids}'" }' "http://10.10.15.52:8888/conch/notice/ding"

exit 0
