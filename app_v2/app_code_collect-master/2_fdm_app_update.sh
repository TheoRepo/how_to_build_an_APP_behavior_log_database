...
from
    ...
    version_no
        from
        profile${label}.dwd_app_20211030 where version_no='${version_no}' and (the_date rlike '${the_date_1}' or the_date rlike '${the_date_2}') and app_name!='14667'
    )a 
...
if [[ $? != 0 ]];then
    msg="定时调度【${version_no}版本规则对最近15天的APP行为按月汇总】跑数失败了55555"
